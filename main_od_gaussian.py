...
def attack(model, train_data, config, hw_array):
    env = Environment_gaussian.Env(model, config=config)
    ...
    for n, _ in tqdm(enumerate(train_data), desc="Clean image analysis", total=len(train_data)):
        ...
        env.query_counts[n] += 1  # Count query for clean
        boxes = results[0].boxes.xyxy.cpu().numpy()
        confs = results[0].boxes.conf.cpu().numpy()
        if len(confs) > 0:
            chosen_indices = confs.argsort()  # All boxes, sorted ascending (lowest conf first)
            chosen_boxes = boxes[chosen_indices]
            allowed_set = set()
            max_allowed = int(img_h * img_w * config["pert_area_fraction"])
            for box in chosen_boxes:
                ...
                for y in range(y1, y2 + 1):
                    for x in range(x1, x2 + 1):
                        if len(allowed_set) < max_allowed:
                            allowed_set.add((x, y))
                        else:
                            break
                    if len(allowed_set) >= max_allowed:
                ...
                if len(allowed_set) >= max_allowed:
                    break
            image_allowed_pixels[n] = list(allowed_set)
        else:
            image_allowed_pixels[n] = []
    ...
    update_images = torch.tensor(np.array(train_data), dtype=torch.float32).clone().to(config["device"])
    metric_images = torch.tensor(np.array(train_data), dtype=torch.float32).clone()
    trick_element = torch.zeros(len(train_data))
    yolo_list = torch.tensor(np.array(train_data), dtype=torch.float32).clone()
    cls_list = []
    ...
    for p in range(config["bound"]):
        ...
        while 1:
            ...
            for bt in tqdm(range(bts), desc=f"RL steps (Cycle {p+1}, Episode {inner_rl_episode})", leave=False):
                ...
                actions = actions.reshape(s.shape[0], config["attack_pixel"], 5)  # Reshape to [batch, attack_pixel, dim]
                # Removed reshape: actions are now [batch, 6] for noise params
                actions_logprob = actions_logprob.reshape(s.shape[0], -1).sum(dim=1)  # Sum logprobs per sample

                allowed_pixels_batch = [image_allowed_pixels[idx] for idx in batch_indices]
                if config["shape_unity"]:
                    rewards, dif_list, changed_images = env.yolo_step_not_sub(s, actions, bt, labels, probs, allowed_pixels_batch, batch_indices=batch_indices)
                else:
                    rewards, dif_list, changed_images = env.yolo_step_disunity_not_sub(s, actions, bt, hws, labels, probs, allowed_pixels_batch, batch_indices=batch_indices)
            ...
            update_images = torch.cat((update_images.unsqueeze(dim=0), torch.tensor(np.array(change_train_x), dtype=torch.float32).to(config["device"]).unsqueeze(dim=0)), dim=0)
            ...
            if delta_box.sum() > 0:
                ...
                for _ in change_idx:
                    yolo_list[_] = torch.tensor(change_train_x[_], dtype=torch.float32).clone()
                    iteration[_] = it
            box_count += delta_box
    ...
    for _ in range(len(yolo_list)):
        if config["shape_unity"]:
            adv_img_array = yolo_list[_].cpu().numpy().astype(np.uint8)  # Ensure numpy for saving
            clean_img_array = metric_images[_].cpu().numpy().astype(np.uint8)
            result = model(adv_img_array, conf=config["yolo_conf"], verbose=False)
            # Compute perturbation magnitude (mean abs diff across RGB)
            ...
            result = model(adv_img_array, conf=config["yolo_conf"], verbose=False)
            # Compute on cropped region
            pert = np.abs(clean_img_array - adv_img_array.astype(np.float32)).mean(axis=2)

        # Normalize perturbation (0-1) for colormap
        if pert.max() > 0:
            pert_norm = pert / pert.max()
        else:
            pert_norm = pert  # All zeros, no changes

        # Save adversarial image
        img = Image.fromarray(adv_img_array)
        img.save(adv_path + f'adv_{_+1:04}.png', 'PNG')

        # Save heatmap (no figure creation needed)
        plt.imsave(delta_path + f'heatmap_{_+1:04}.png', pert_norm, cmap='hot')

        # Save adversarial result with detections
        im_array = result[0].plot()
        im = Image.fromarray(im_array)
        im.save(f'{adv_result_path}adv_{_+1:04}.png', 'PNG')

    # Save query counts (iterations as proxy for queries per image)
    QUERY_FILE = os.path.join(adv_path, 'queries.npy')  # Or use your preferred path
    np.save(QUERY_FILE, iteration.cpu().numpy())
...
if __name__ == '__main__':
    ...
    config["bound"] = 50
    config["limit"] = 5e-2
    config["attack_pixel"] = 1  # For Gaussian noise, single action per image
    config["yolo_conf"] = 0.50
    config["patient"] = 20
    config["classifier"] = "yolo"
    config["dataset"] = "BDD100K"  # <-- Set for your project
    config["pert_area_fraction"] = 0.05  # 5% of image pixels; tune as needed
    ...
    # Result path
    result_path = "/content/drive/MyDrive/Project_RLAB/Main/Results_Gaussian/original_result"
    adv_path = "/content/drive/MyDrive/Project_RLAB/Main/Results_Gaussian/adv_images"
    adv_result_path = "/content/drive/MyDrive/Project_RLAB/Main/Results_Gaussian/adv_result"
    delta_path = "/content/drive/MyDrive/Project_RLAB/Main/Results_Gaussian/delta_images"
    ...
    # Removed: config["attack_pixel"] = int((img_hw_list.max(axis=0)[0] + img_hw_list.max(axis=0)[1]) / 2 * config["attack_pixel"])  # No longer needed for pixel count
