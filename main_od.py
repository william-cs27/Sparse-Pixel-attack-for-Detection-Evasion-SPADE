...
def attack(model, train_data, config, hw_array):
    ...
    for _ in range(len(yolo_list)):
        if config["shape_unity"]:
            adv_array = yolo_list[_].clip(0, 255).byte().cpu().numpy()
            delta_array = torch.abs(metric_images[_] - yolo_list[_]).clip(0, 255).byte().cpu().numpy()
            img = Image.fromarray(adv_array)
            delta_img = Image.fromarray(delta_array)
            result = model(adv_array, conf=config["yolo_conf"], verbose=False)
        else:
            adv_cropped = yolo_list[_, :hw_array[_,0], :hw_array[_,1]].clip(0, 255).byte().cpu().numpy()
            delta_array = torch.abs(metric_images[_, :hw_array[_,0], :hw_array[_,1]] - yolo_list[_, :hw_array[_,0], :hw_array[_,1]]).clip(0, 255).byte().cpu().numpy()
            img = Image.fromarray(adv_cropped)
            delta_img = Image.fromarray(delta_array)
            result = model(adv_cropped, conf=config["yolo_conf"], verbose=False)
...
if __name__ == '__main__':
    ...
    # Model
    model = YOLO('/content/drive/MyDrive/Project_RLAB/RFPAR/runs/detect/train2/weights/best.pt').to('cuda') # <-- Use your own trained YOLOv8n weights for BDD100K
